...
public class ConfigurationJFrame extends FluoJJFrame implements ActionListener
{
	...
	private void initComponents()
	{
		...
		iconlb = new JLabel(Sample.getDefaultIcon());
		imgpn.add(iconlb);
	...
	private void addListeners()
	{
		samplestb.getSelectionModel().addListSelectionListener(new ListSelectionListener()
		{
			...
			public void valueChanged(ListSelectionEvent e)
			{
				...
				{
					sample = null;
					iconlb.setIcon(Sample.getDefaultIcon());
				}
		...
		activeimgbt.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				try
				{
					processImageParticles(new FluoJImageProcessor(img, sample, false, cconfigurationdb.isDebug()), sample.getSamplefeatureList(), sample.getSamplecorefeatureList());
				}
				catch (InvalidOperationOnResourceException e1)
				{
	...
	void processImageParticles() throws InvalidOperationOnResourceException
	{
		...
		processImageParticles(getCImageProcess(), sample.getSamplefeatureList(), sample.getSamplecorefeatureList());

	}
	
	void processImageParticles(List<Samplefeature> sfs, List<Samplecorefeature> scfs) throws InvalidOperationOnResourceException
	{
	...
	public void processImageParticles(FluoJImageProcessor cip, List<Samplefeature> sfs, List<Samplecorefeature> scfs) throws InvalidOperationOnResourceException
	{
	...
	public FluoJImageProcessor getCImageProcess() throws InvalidOperationOnResourceException
	{
		if (cip == null)// reseted or first time
			cip = new FluoJImageProcessor(sample.getImageresource().getImagePlus(), sample, false, cconfigurationdb.isDebug());
		...
		public void setValueAt(Object value, int rowIndex, int columnIndex)
		{
			try
			{
				...
				else if (columnIndex == 5)
				{
					...
					if (result == JOptionPane.YES_OPTION)
					{
						sample.setSeparation(separation);
						resetCImageProcess();
					}
				...
				{
					Integer expansionradius = (Integer) value;
					result = resetDB();
					if (result == JOptionPane.YES_OPTION)
					{
		...
		public Object getValueAt(int rowIndex, int columnIndex)
		{
			...
			if (columnIndex == 1)
			{
				if (sample.getSession() == null)
					...
					return sample.getSession();

			}
		
			if (columnIndex == 2)
				return sample.getThreshold();
			
			if (columnIndex == 3)
				return sample.getRoisthreshold();

			if (columnIndex == 4)
				return (sample.getFillholes() == 1) ? true : false;
			if (columnIndex == 5)
				return sample.getSeparation();
			
			if (columnIndex == 6)
				return 
						sample.getExpansionradius();
			if (columnIndex == 7)
	...
	class TypeTableCellEditor extends AbstractCellEditor implements TableCellEditor
	{
		...
		public TypeTableCellEditor()
		{
			...
			cb.addActionListener(new ActionListener()
			{
				...
				public void actionPerformed(ActionEvent e)
				{
					...
					if ((type.equals(nonetype) && sample.getType() == null) || type.equals(sample.getType()))
		...
		public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column)
		{
			...
			if (sample.getType() != null)
				cb.setSelectedItem(sample.getType());
			else
	...
	class SessionTableCellEditor extends AbstractCellEditor implements TableCellEditor
	{
		...
		public SessionTableCellEditor()
		{
			...
			cb.addActionListener(new ActionListener()
			{
				...
				public void actionPerformed(ActionEvent e)
				{
					...
					if ((session.equals(nonesession) && sample.getSession() == null) || session.equals(sample.getSession()))
						return;// nothing to do here
		...
		public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column)
		{
			...
			if (sample.getSession() != null)
				cb.setSelectedItem(sample.getSession());
			else
	...
	class SeparateTableCellEditor extends AbstractCellEditor implements TableCellEditor
	{
		...
		{
			sample = ConfigurationJFrame.this.samples.get(row);
			cb.setModel(new DefaultComboBoxModel(cconfigurationdb.getSeparations(em).toArray()));
			cb.setSelectedItem(sample.getSeparation());
			return cb;
		}
