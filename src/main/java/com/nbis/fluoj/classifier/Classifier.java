...
public class Classifier implements IClassifier
{
	...
	{
		return getTotalNormal(em) + getTotalFragmented(em);
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see classifier.IClassifier#getTotalNormal(EntityManager)
	 */
	@Override
	public int getTotalNormal(EntityManager em)
	{
		int total = cdb.getScellTotalForWinner(5, em);
		total += cdb.getScellTotalForWinner(7, em) * 2; // NormalNormal
		total += cdb.getScellTotalForWinner(8, em); // NormalFragm
		return total;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see classifier.IClassifier#getTotalFragmented(EntityManager)
	 */
	@Override
	public int getTotalFragmented(EntityManager em)
	{
		int total = cdb.getScellTotalForWinner(6, em);
		total += cdb.getScellTotalForWinner(9, em) * 2; // FragmFragm
		total += cdb.getScellTotalForWinner(8, em); // NormalFragmType
		return total;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see classifier.IClassifier#getTotalDegraded(EntityManager)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see classifier.IClassifier#getSession()
	 */
	@Override
	...
	public int persistCellsOnImage(List<Scell> scells, ImagePlus img, EntityManager em)
	{
		...
		Imageresource ir = cdb.saveImageResource(img, em);
		for (int i = 0; i < scells.size(); i++)
			scells.get(i).setImageresource(ir);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see classifier.IClassifier#getTotalDyedParticles(EntityManager em)
	 */
	@Override
	public int getTotalDyedParticles(EntityManager em)
	{
		return cdb.getScellTotalForWinner(10, em);
	}
